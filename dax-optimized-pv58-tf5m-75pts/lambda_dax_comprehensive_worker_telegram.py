#!/usr/bin/env python3
"""
DAX Strategy - Comprehensive Worker Lambda Function
===================================================

This worker processes a chunk of parameter combinations for the comprehensive
DAX strategy with ALL possible parameter combinations including bidirectional trading.

Key Features:
- Processes 25 parameter combinations per invocation
- Supports bidirectional trading parameter
- Comprehensive parameter space testing
- Optimized for Lambda execution limits
- Handles all parameter permutations

Expected runtime: ~3-8 minutes per chunk (comprehensive testing)
"""
...
class StrategyConfig:
    """Comprehensive strategy configuration with bidirectional option"""
    def __init__(self, **kwargs):
        ...
        self.dual_position = kwargs.get('dual_position', False)
        self.stop_mode = kwargs.get('stop_mode', 'zone')
        self.bidirectional = kwargs.get('bidirectional', False)  # NEW PARAMETER
...
class SignalDetector:
    """Comprehensive signal detection with bidirectional option"""
    def __init__(self, config):
        self.pending_signal = None
        self._confirmed_signal = None
        self.bidirectional = config.bidirectional  # Store bidirectional setting
    ...
    def check_signal(self, candle, zones, occupied_sides):
        ...
        for zone_name in ["sup1", "res1"]:
            ...
            # NEW: Comprehensive signal logic based on bidirectional setting
...
class PositionManager:
    """Position management with comprehensive options"""
    def __init__(self, config):
        self.config = config
        self.positions = {}
...
class DAXStrategyEngineComprehensive:
    """Comprehensive DAX strategy engine with bidirectional option"""
    def __init__(self, config):
        self.config = config
        self.zone_engine = SRZoneEngine(config.pivot_left, config.pivot_right)
        self.signal_detector = SignalDetector(config)  # Pass config for bidirectional
    ...
    def run(self, df_1min):
        """Run strategy with comprehensive options"""
...
def run_strategy_test(config_dict, data):
    """Run strategy test with given comprehensive configuration"""
    try:
...
def lambda_handler(event, context):
    """Main Lambda handler for comprehensive worker"""
    ...
    try:
        ...
        print(f"🔄 Processing chunk {chunk_id} with {len(combinations)} comprehensive combinations")
...
if __name__ == "__main__":
    ...
    # Test with sample comprehensive configurations
    test_event = {
        'chunk_id': 0,
        'combinations': [
            {
            ...
            },
            {
                ...
                'force_exit_time': "16:30", 'dual_position': True,
